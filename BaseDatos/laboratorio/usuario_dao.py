...
if __name__ == '__main__':
    ...
    #usuario = Usuario(username='cristyan' ,password='45678')
    #usuarios_insertados = UsuarioDao.insertar(usuario)
    #logger.debug(f'Personas insertadas :{usuarios_insertados}')
    
    usuario = Usuario(2,'juan' , 'per123')
    usuarios_editados = UsuarioDao.actualizar(usuario)
    logger.debug(f'Personas actualizadas : {usuarios_editados} ')
    
    
    usuario = Usuario(id_usuario=4)
    usuarios_eliminados = UsuarioDao.eliminar(usuario)
    logger.debug(f'Personas eliminadas : {usuarios_eliminados} ')
